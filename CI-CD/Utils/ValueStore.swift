...
final class ValueStore: ObservableObject {
    ...
    func bumpRefreshToken(for provider: AccountProvider) {
        switch provider {
        case .connect: connectRefreshToken = UUID()
        case .coolify: coolifyRefreshToken = UUID()
        case .github: githubRefreshToken = UUID()
        }
    }
    ...
    private func selectedID(for provider: AccountProvider) -> UUID? {
        switch provider {
        case .connect: selectedConnectAccountID
        case .coolify: selectedCoolifyAccountID
        case .github: selectedGitHubAccountID
        }
    }
